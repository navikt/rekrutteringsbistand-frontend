...
export default function formaterMedStoreOgSmåBokstaver(
  tekst?: string | null,
): string | undefined {
...
const gjørFørsteBokstavEtterSeparatorStor = (
  setning: string,
  separator: string,
): string => {
...
const formaterOrd = (ord: string): string => {
  if (skalVæreMedStoreBokstaver.includes(ord)) {
    ...
    return førsteBokstav === undefined
      ? ord
      : førsteBokstav.toUpperCase() + ord.substring(1);
  } else {
    return ord;
  }
};
