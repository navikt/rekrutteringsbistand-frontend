...
export default function ViktigeDatoer({ skjulFrist }: Props) {
  ...
  // Synkroniser formverdier dersom vi formatterer (unngå hopp ved første render)
  useEffect(() => {
        ...
        <div className='flex flex-col'>
          ...
          <div className='flex gap-8 items-center'>
            <DatoVelger
              ...
              setDato={(val) => {
                if (val) {
                  setValue('stilling.properties.starttime', val as any);
                }
              }}
            />
            <Controller
                ...
                render={({ field: { value, onChange } }) => (
                  <Checkbox
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        onChange('Snarest');
                        // Sett en veiledende "avsluttes"/expires-dato dersom backend forventer det (3 uker frem)
                        setValue(
                          'stilling.expires',
                          format(
                            addWeeks(new Date(), 3),
