...
} from '@/app/rekrutteringstreff/[rekrutteringstreffId]/_ui/treffgjennomforing/treffgjennomforingNavn';
...
const Oppmøte: FC<Props> = ({
  ...
  const fjernOppmøte = async (
    ...
    try {
      const oppdatertTreffgjennomforing = await oppdaterOppmøte(
        rekrutteringstreffId,
        personTreffId,
        false,
  ...
  const startFjernOppmøte = (personTreffId: string, navn: string) => {
    if (
      harRegistreringer(tellRegistreringer(treffgjennomforing, personTreffId))
    ) {
  ...
  return (
    <VStack gap='space-32'>
      ...
      <HGrid columns={{ xs: 1, lg: 2 }} gap='space-24'>
        <section aria-labelledby='treffgjennomforing-oppmøte-heading'>
          <Heading
            id='treffgjennomforing-oppmøte-heading'
            level='3'
            size='small'
            spacing
          >
        ...
        <section aria-labelledby='treffgjennomforing-arbeidsgivere-heading'>
          <Heading
            id='treffgjennomforing-arbeidsgivere-heading'
            level='3'
            size='small'
            spacing
          >
