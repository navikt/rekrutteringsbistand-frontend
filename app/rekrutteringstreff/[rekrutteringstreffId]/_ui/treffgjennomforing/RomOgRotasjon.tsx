...
import { oppdaterRomfordeling } from '@/app/api/rekrutteringstreff/[...slug]/treffgjennomforing/mutations';
import {
  beregnRotasjonsplan,
...
import Møteoppsettpanel from '@/app/rekrutteringstreff/[rekrutteringstreffId]/_ui/treffgjennomforing/Møteoppsettpanel';
...
import { settDragImage } from '@/app/rekrutteringstreff/[rekrutteringstreffId]/_ui/treffgjennomforing/dragImage';
...
import { useUtskrift } from '@/app/rekrutteringstreff/[rekrutteringstreffId]/_ui/treffgjennomforing/useUtskrift';
import {
  lagArbeidsgiverplaner,
  lagRomplaner,
} from '@/app/rekrutteringstreff/[rekrutteringstreffId]/_ui/treffgjennomforing/utskriftsplan';
...
const RomOgRotasjon: FC<Props> = ({
  ...
  const visteRom = optimistiskeRom ?? treffgjennomforing.rom;
  ...
  const rotasjonsplan = beregnRotasjonsplan(
    treffgjennomforing.arbeidsgiverRekkefølge,
    treffgjennomforing.antallRom,
    treffgjennomforing.starttidspunkt,
    treffgjennomforing.varighetPerMøteMinutter,
  );
  ...
  const arbeidsgiverplaner = lagArbeidsgiverplaner(
    rotasjonsplan,
    treffgjennomforing.arbeidsgiverRekkefølge.map(
  ...
  const flyttOgLagre = async (personTreffId: string, målromnummer: number) => {
    ...
    try {
      const oppdatertTreffgjennomforing = await oppdaterRomfordeling(
        rekrutteringstreffId,
        oppdaterteRom,
      );
  ...
  return (
    <VStack gap='space-32'>
      ...
      <Møteoppsettpanel
        ...
        onTreffgjennomforingOppdatert={onTreffgjennomforingOppdatert}
        deaktivert={lagrerRom}
      />
