...
export default function Tidspunkt({
  ...
  useEffect(() => {
    if (periodUgyldig) {
      ...
      setError('tilDato', { type: 'visualCueOnly', message: '' });
      setError('tilTid', { type: 'visualCueOnly', message: '' });
    } else {
      clearErrors(['root' as any, 'tilDato', 'tilTid']);
    }
  }, [periodUgyldig, setError, clearErrors]);
