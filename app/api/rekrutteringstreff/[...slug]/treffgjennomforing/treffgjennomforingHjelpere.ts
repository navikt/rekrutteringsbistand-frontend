...
export const fordelIntervjuerForenklet = (
  ...
  const utgangspunkt = arbeidsgiverTreffIder.map((arbeidsgiverTreffId) => {
    const lagret = treffgjennomforing.intervjufordelinger.find(
      (fordeling) => fordeling.arbeidsgiverTreffId === arbeidsgiverTreffId,
    );
    const ønskede = treffgjennomforing.interesser
      .filter(
...
export interface Treffgjennomforingsregistreringer {
  interesser: number;
...
export const tellRegistreringer = (
  ...
  personTreffId: string,
): Treffgjennomforingsregistreringer => {
  if (!treffgjennomforing) {
    return { interesser: 0, intervjuplasser: 0, vurderinger: 0 };
  }

  const interesser = treffgjennomforing.interesser.filter(
    (interesse) => interesse.personTreffId === personTreffId,
  ).length;

  const intervjuplasser = treffgjennomforing.intervjufordelinger.filter(
    (fordeling) =>
      fordeling.inkludertePersonTreffIder.includes(personTreffId) ||
      fordeling.ekskludertePersonTreffIder.includes(personTreffId),
  ).length;

  const vurderinger = treffgjennomforing.vurderinger.filter(
    (vurdering) =>
...
export const harRegistreringer = (
  registreringer: Treffgjennomforingsregistreringer,
): boolean =>
