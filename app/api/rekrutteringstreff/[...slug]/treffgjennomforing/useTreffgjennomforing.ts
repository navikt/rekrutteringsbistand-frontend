...
const TreffgjennomforingFaseSchema = z.enum([
  'OPPMØTE',
  'ROM',
  'INTERESSE',
  'FORDELING',
...
export const TreffgjennomforingSchema = z.object({
  rekrutteringstreffId: z.string(),
  fase: TreffgjennomforingFaseSchema,
  antallRom: z.number().int().min(1),
  starttidspunkt: z.string().regex(KLOKKESLETT_REGEX),
  varighetPerMøteMinutter: z.number(),
  oppmøte: z.array(z.string()),
  // Valgfritt inntil backend har feltet, slik at treffgjennomforingen ikke velter mot en
...
export type TreffgjennomforingFase = z.infer<
  typeof TreffgjennomforingFaseSchema
>;
...
export type TreffgjennomforingDTO = z.infer<typeof TreffgjennomforingSchema>;
...
export const treffgjennomforingEndepunkt = (id: string) =>
  `${RekrutteringstreffAPI.internUrl}/${id}/treffgjennomforing-og-oppfolging`;

export const treffgjennomforingSkrivEndepunkt = (id: string) =>
...
export const useTreffgjennomforing = (id: string | undefined) =>
  useSWRGet(
    id ? treffgjennomforingEndepunkt(id) : null,
